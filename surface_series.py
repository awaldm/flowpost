...
if __name__ == "__main__":
    ...
    data_type = 'OAT15A'
    plane = 'wing'
    image_series_folder = case_name + '_wing_ss_images'
    ...
    if write_images:
        print('writing images to ' + image_series_folder)
        # do image series
        import plot_CRM_wing_contour as surfplotter
        print cp.shape[1]
        print range(cp.shape[1])
        for i in range(cp.shape[1]):
            print i
            #fig, ax = surfplotter.plot_surface(y, -x, cp[:,i])
            fig, ax = surfplotter.plot_surface(y, -x, cp[:,i], kind='OAT_cpmean', shape=None, struct=False)
            count = start_i + (i*10)
            ax.text(0.9, 0.9,str(count).zfill(4), horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
            print 'creating figure ' + str(count)
            adjustprops = dict(right=0.9)       # Subplot properties
            plt.subplots_adjust(**adjustprops)
            plt.savefig(image_series_folder + '/' + case_name+'_wing_'+str(count).zfill(4)+'.png', dpi=600)
            plt.close()
